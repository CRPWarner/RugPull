...
contract WeGro is ERC20, Ownable {
    ...
    WeGroDividendTracker public dividendTracker;

    address public constant deadWallet =
        0x000000000000000000000000000000000000dEaD;

    uint256 private constant _totalSupply = 8400000000 * (10**18);
    uint256 public constant swapTokensAtAmount = 84000 * (10**18);
    ...
    address private _marketingWalletAddress = 0xD6B78D20c9C774abf47FA83FdD1C4e3504CB89C9;
    ...
    constructor() public ERC20("WeGro", "WEGRO") {
        dividendTracker = new WeGroDividendTracker();
    ...
    function prepForLaunch() public onlyOwner {
        require(
            !tradingReady,
            "WeGro: Contract has already been prepped for trading."
        );
        ...
        */
        ...
        launchSellFeeDeadline = now + 3 days; // sell penalty for the first 24 hours
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(
            ...
            "WeGro: Account is already the value of 'excluded'"
        );
        _isExcludedFromFees[account] = excluded;
    ...
    function setMarketingWallet(address payable wallet) external onlyOwner {
        require(wallet != address(0), "WeGro: Marketing wallet cannot be 0!");
        require(
            wallet != deadWallet,
            "WeGro: Marketing wallet cannot be the dead wallet!"
        );
        _marketingWalletAddress = wallet;
    }

    function setLaunchSellFee(uint256 newLaunchSellFee) external onlyOwner {
        require(newLaunchSellFee <= 25, "WeGro: Maximum launch sell fee is 25%");
        launchSellFee = newLaunchSellFee;
        emit LaunchFeeUpdated();
    }

    function setFees(uint256 ethRewards, uint256 liquidity, uint256 marketing) external onlyOwner {
        require(ethRewards <= 10, "WeGro: Maximum ETH Reward fee is 10%");
        require(liquidity <= 5, "WeGro: Maximum liquidity fee is 5%");
        require(marketing <= 5, "WeGro: Maximum Marketing fee is 5%");
        ...
        onlyOwner
    {
        require(
            pair != uniswapV2Pair,
            "WeGro: The Uniswap pair cannot be removed from automatedMarketMakerPairs"
        );
    ...
    function blacklistAddress(address account, bool value) public onlyOwner {
        if (value) {
            require(
                ...
                "WeGro: The ability to blacklist accounts has been disabled."
            );
        }
    ...
    function setMaxTxPercent(uint256 newMaxTx) public onlyOwner {
        require(newMaxTx >= 5, "WeGro: Max TX should be above 0.5%");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(
            ...
            "WeGro: Automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[pair] = value;
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(
            ...
            "WeGro: gasForProcessing must be between 200,000 and 500,000"
        );
        require(
            newValue != gasForProcessing,
            "WeGro: Cannot update gasForProcessing to same value"
        );
    ...
    function _transfer(
        ...
        if (
            ...
            require(
                amount <= maxTxAmount,
                "WeGro: Transfer amount exceeds the maxTxAmount."
            );
        }
...
contract WeGroDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    ...
    constructor() public DividendPayingToken("WeGroDvt", "WeGroDvt") {
        claimWait = 3600;
    ...
    function _transfer(
        ...
        require(false, "WeGro_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public override {
        require(
            false,
            "WeGro_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main WeGro contract."
        );
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(
            ...
            "WeGro_Dividend_Tracker: claimWait must be updated to between 30 minutes and 24 hours"
        );
        require(
            newClaimWait != claimWait,
            "WeGro_Dividend_Tracker: Cannot update claimWait to same value"
        );
