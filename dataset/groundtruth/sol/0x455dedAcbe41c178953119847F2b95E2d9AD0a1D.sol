...
contract $BNBDeFiDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("$BNBDeFi_Dividend_Tracker", "$BNBDeFi_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**18); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "$BNBDeFi_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "$BNBDeFi_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main $BNBDeFi contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "$BNBDeFi_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "$BNBDeFi_Dividend_Tracker: Cannot update claimWait to same value");
...
Token name: BNB DeFi 
Symbol: $DEFI
New owner Address: 0x2541a4f3d2908f169EC6D3E9dfe8C87e88E2bd7b
New marketing Address: 0x1C1bBafD23140b4B32b593a7439f63bAfC1124e8
Changes requested : 
1:Owner can't set buy few more then 20% 
2: owner can't set sell fee more then 20% 
3: Owner can't change max TX
*/
contract BNBDeFiToken is ERC20, Ownable, SafeToken {
    using SafeMath for uint256;
    ...
    $BNBDeFiDividendTracker public dividendTracker;
    ...
    constructor() ERC20("BNBDeFi", "$DEFI") {
        BNBRewardsFee = 3;
        liquidityFee = 3;
        marketingFee = 3;
    	...
    	dividendTracker = new $BNBDeFiDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "$BNBDeFi: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "$BNBDeFi: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "$BNBDeFi: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "$BNBDeFi: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "$BNBDeFi: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "$BNBDeFi: Cannot update gasForProcessing to same value");
